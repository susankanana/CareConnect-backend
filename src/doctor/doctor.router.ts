...
import { bothRoleAuth, allRoleAuth, userRoleAuth } from '../middleware/bearerAuth';
...
const doctor = (app: Express) => {
  ...
  app.route('/doctor/:id').get(allRoleAuth, async (req, res, next) => {
    try {
  ...
  app.route('/doctors/specialization/:specialization').get(userRoleAuth, async (req, res, next) => {
    try {
