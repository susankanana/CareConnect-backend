...
export const createUserService = async (user: TIUser) => {
  const { specialization, availableDays, experience, patients, rating, ...userData } = user;
  ...
  if (isNowDoctor) {
    ...
    if (doctorRecord) {
      if (Object.keys(doctorData).length > 0) {
        await db.update(DoctorsTable).set(doctorData).where(eq(DoctorsTable.doctorId, id));
      }
    } else {
      if (!specialization || !availableDays) {
